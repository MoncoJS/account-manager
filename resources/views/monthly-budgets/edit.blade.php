...
<div class="card shadow-sm">
    <div class="card-header bg-white py-3">
        <h4 class="mb-0">
            <i class="bi bi-wallet2 text-primary"></i> แก้ไขงบประมาณรายเดือน
        </h4>
    </div>
    <div class="card-body">
        @if($errors->any())
        ...
        <form action="{{ route('monthly-budgets.update', $monthlyBudget) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                    ...
                    <select class="form-select @error('category_id') is-invalid @enderror" 
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" 
                                    {{ old('category_id', $monthlyBudget->category_id) == $category->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <div class="input-group">
                        <input type="number" 
                               ...
                               id="amount" 
                               name="amount" 
                               value="{{ old('amount', $monthlyBudget->amount) }}" 
                               step="0.01" 
                               required>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('month') is-invalid @enderror" 
                        ...
                        @foreach(range(1, 12) as $month)
                            <option value="{{ $month }}" 
                                    {{ old('month', $monthlyBudget->month) == $month ? 'selected' : '' }}>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('year') is-invalid @enderror" 
                        ...
                        @foreach(range(date('Y'), date('Y')-5) as $year)
                            <option value="{{ $year }}" 
                                    {{ old('year', $monthlyBudget->year) == $year ? 'selected' : '' }}>
                                {{ $year }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-12 mb-3">
                    <div class="form-check">
                        <input type="checkbox" 
                               ...
                               value="1" 
                               {{ old('notify_when_exceeded', $monthlyBudget->notify_when_exceeded) ? 'checked' : '' }}>
